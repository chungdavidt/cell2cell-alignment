...
% marker, QC pair and rolony cutoff.
%
...
%   <Marker>_plots_dtc\qc<reads>_<genes>\full\ge<cut>_sat<cap>\
...
GENES    = 5:-1:0;
ROLONIES = 0:3;
EXPECTED_RUNS = 240;   % numel of the four lists multiplied; update with them
...
n_runs = numel(MARKERS) * numel(READS) * numel(GENES) * numel(ROLONIES);
assert(n_runs == EXPECTED_RUNS, ...
    ['The lists give %u runs, EXPECTED_RUNS says %u. Check MARKERS, READS, ' ...
     'GENES and ROLONIES, then update EXPECTED_RUNS.'], n_runs, EXPECTED_RUNS);
    ...
    for reads = READS(:)'
        for genes = GENES(:)'
            for rolonies = ROLONIES(:)'
                ...
                fprintf('[%3u/%u] %-8s qc%g_%g ge%g  %-7s %6.1f s  (elapsed %s)  %s\n', ...
                    k, n_runs, marker{1}, reads, genes, rolonies, status, ...
