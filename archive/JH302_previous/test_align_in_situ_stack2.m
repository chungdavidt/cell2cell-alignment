...
for nn=cell2mat(filt_neurons.uniq_slice{i_slice})'
    ...
    for i_fov = 1:length(fov_names)
        ...
        %hyb_file2 = ['.\hyb\',fov_names{i_fov+1},'\alignedn2vhyb01.tif'];
        %im_hyb2 = imread(hyb_file2);

%         fname = ['.\hyb\',fov_names{i_fov}];
        %fname{2} = hyb_file2;
        
        %stitch_10x_images_mist(fname,1,.23,1);%,xcam,ycam,mock)
...
%         figure;
%         imagesc(im_hyb); hold on

        xxx = find(filt_neurons.fov==fov_id(i_fov));
%         %figure
%         plot(filt_neurons.pos(xxx,1)*2-x_offset,filt_neurons.pos(xxx,2)*2-y_offset, '.r');

%         figure
%         imagesc(im_hyb); hold on
%         plot(filt_neurons.pos40x(xxx,1),filt_neurons.pos40x(xxx,2), '.r');
        
        b = regress(double(filt_neurons.pos(xxx,1))*2,[ones(size(filt_neurons.pos40x(xxx,1))) double(filt_neurons.pos40x(xxx,1))]);
        x_offset_new = round(b(1));
        b = regress(double(filt_neurons.pos(xxx,2))*2,[ones(size(filt_neurons.pos40x(xxx,2))) double(filt_neurons.pos40x(xxx,2))]);
...
%         [y_offset  y_offset_new]
        ...
        im_tmp2(y_offset_new+1:y_offset_new+3200, x_offset_new+1:x_offset_new+3200) = im_hyb;

        im_tmp3(y_offset_new+1:y_offset_new+3200, x_offset_new+1:x_offset_new+3200,2) = im_hyb;
        im_tmp3=max(im_tmp3,[],3);
