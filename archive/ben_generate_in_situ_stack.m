...
% 
%         fov_names = unique(filt_neurons.fov_names(filt_neurons.fov(in_slice)));
% 
%     im_tmp = uint16(1);
% 
...
%     raw_image_stack{n_raw_im,2} = [filt_neurons.pos(in_slice,1)*2 filt_neurons.pos(in_slice,2)*2];
% end
...
% preallocate file
m = matfile('stitched_HYB.mat', 'Writable', true);
...
% load metadata about each neuron
load('./filt_neurons.mat');
...
for nn=cell2mat(filt_neurons.uniq_slice{i_slice})'
    ...
    disp(['stitching slice ',num2str(nn)])
    n_raw_im = n_raw_im+1;
    ...
    for i_fov = 1:length(fov_names)
        ...
        cell_ids = (filt_neurons.id(i_cell));     % find all cell ids in FOV
        % cell_ids_one_index = uint16(cell_ids - min(cell_ids)) + 1;    % have them start at index 1
        % [mscarlet_cells, ~] = find(countspercell(i_cell)); % find cells with mscarlet xpression

        % if isempty(cell_ids_one_index(mscarlet_cells))
        % else
        %     for id = cell_ids_one_index(mscarlet_cells)'
        %         im_hyb(im_hyb == id) = 65535;
        %     end
        % end

        im_hyb(im_hyb > 0 & im_hyb ~= 65535) = 255;
        % im_hyb(im_hyb == 65535) = 128;
...
for i_im = 1:length(raw_image_stack)
    im1 = raw_image_stack{i_im,1};      % fixed
    % im2 = raw_image_stack{i_im,1};        % moving

    % trim size
    x_offset = min(raw_image_stack{i_im,2}(:,1));
    y_offset = min(raw_image_stack{i_im,2}(:,2));
    if x_offset + 8960 > width(im1)
        x_offset = width(im1) - 8960;
    end

        if y_offset + 11840 > length(im1)
        y_offset = length(im1) - 11840;
    end

    im1 = im1((1:11840)+y_offset, (1:8960)+x_offset);
    im1(im1>255) = 255;
   
    % x_offset = min(raw_image_stack{i_im,2}(:,1));
    % y_offset = min(raw_image_stack{i_im,2}(:,2));
    % im2 = im2((1:11840)+y_offset, (1:8960)+x_offset);
    figure
    image(im1)
    ...
    ax.Visible = 'off';
    imsave(gcf)
    index = "JH302_" + string(i_slice) + "_" + string(i_im);
    m.(index) = im1;
end
